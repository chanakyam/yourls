...
<h2 id="informations"><?php echo yourls_esc_html( "Statistics" ); ?></h2></br>
...
<?php if( yourls_do_log_redirect() ) { ?>
	<div id="stat_tab_stats" class="tab">
		...
		<?php if ( $list_of_days ) { ?>
			...
			<tr>
				<td valign="top">
				<ul id="stats_lines" class="toggle_display stat_line TSList">
				<?php
				if( $do_24 == true )
		...
		<?php if ( $referrers ) { ?>
			...
			<tr>
				<td valign="top">
					...
					<ul class="no_bullet">
						...
						foreach( $referrer_sort as $site => $count ) {
							...
							echo "<li class='sites_list'><img src='$favicon' class='fix_images'/> $site: <strong>$count</strong> <a href='' class='details hide-if-no-js' id='more_url$i'>". yourls__( '(For Details)' ) . "</a></li>\n";
							echo "<ol id='details_url$i' style='display:none' class='detail_list'>";
							foreach( $referrers[$site] as $url => $count ) {
								//echo "<li>"; yourls_html_link($url); echo ": <strong>$count</strong></li></br>\n";
								echo "<li>"; yourls_html_link($url); echo ": ". sprintf( yourls_n( '<strong>%s</strong> hit', '<strong>%s</strong> hits', $count ), $count );
							}
							echo "</ol>\n";
						...
						if ( $referrers ) {
							echo "<li id='sites_various'>" . yourls__( 'Various:' ) . " <strong>". count( $referrers ). "</strong> <a href='' class='details hide-if-no-js' id='more_various'>" . yourls__( '(For Details)' ) . "</a></li></br>\n";
							echo "<ol id='details_various' style='display:none'>";
				...
				<td valign="top">
					...
					<?php
					yourls_stats_pie( array( yourls__( 'Dir' ) => $direct, yourls__( 'Ref' ) => $notdirect ), 5, '440x250', 'stat_tab_source_direct' );
					?>
	...
	<div id="stat_tab_fbcount" class="tab">
		...
		<?php
			...
			$arr_keywords = array();
			foreach ($keywords_rows as $keyword_obj) {
				$arr_keywords[] = yourls_link($keyword_obj->keyword);
			}
			$csv_yourls_link = implode(",", $arr_keywords);

			$facebook_api='http://api.facebook.com/restserver.php?method=links.getStats&urls='.$csv_yourls_link.'&format=json';
			$json=file_get_contents($facebook_api);
			$facebook_response = json_decode($json,true);

			foreach ($facebook_response as $facebook_response_value) {
				
				$facebook_share_count = $facebook_share_count+$facebook_response_value['share_count'];
			}

			echo ("Facebook Share Count is : " .$facebook_share_count);
		?>
		...
		foreach ($keywords_rows as $keyword_obj) {
			...
			$twitter_api=file_get_contents('http://urls.api.twitter.com/1/urls/count.json?url='.yourls_link($keyword_obj->keyword));
			$twitter = json_decode($twitter_api, true);
			$twitter_share_count = $twitter_share_count+$twitter['count'];
			
		}

    	echo ("Twitter Share Count is : " .$twitter_share_count );
		?>
